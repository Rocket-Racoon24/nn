...
@token_required
def get_total_time():
    """Get total time spent for the authenticated user"""
    user_email = request.user['email']
    try:
        progress = progress_collection.find_one({"email": user_email})
        
        if not progress:
            # User has no progress record yet (hasn't logged out since progress tracking was added)
            return jsonify({
                "email": user_email,
                "total_time_spent": 0,
...
@token_required
def get_progress():
    """Get complete progress data for the authenticated user"""
    user_email = request.user['email']
    try:
        progress = progress_collection.find_one({"email": user_email})
        ...
        result = {
            ...
            "last_logout_end": progress.get("last_logout_end"),
            "created_at": progress.get("created_at"),
            "last_login_start": progress.get("last_login_start")  # If it exists
        }
